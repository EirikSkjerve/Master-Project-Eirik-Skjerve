...
% \Procedure{Euclid}{$a,b$}\Comment{The g.c.d. of a and b}
    ...
    \State \Return{$\vec{c}$ and $\mat{L}^{-1}$}
\end{algorithmic}
\end{algorithm}
...
so each sample in the vectors $\vec{c}$ has variance $1$. This also aligns with our theoretical analysis of $\mom{4}{C}$. Even though the distribution $\dgdi$ is discrete, the shape of the samples $\vec{c} \in \PP{C}$ will still have a spherical

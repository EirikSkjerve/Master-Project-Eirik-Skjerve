...
A simple gradient descent in this setting is described in the following algorithm:
...
If all rows of $\mat{C}$ are independent of each other, one has to find the correct combination of the rows that constitutes $\mat{C}$. This requires at most $2 \cdot n!$ attempts, which quickly becomes an impossibly large number as $n$ grows. 
The secret key of the prey for the HPP-attack, NTRU-sign, however, does not have independent rows. In fact, finding any one row of $\mat{C}$ reveals the remaining rows with a polynomial number of different combinations.
As we will show, this is in part also the case for Hawk.

...
\begin{algorithmic}[1]
    ...
    \State $\vec{w} \gets \text{ random vector on unit sphere in } \bb{R}^{2n}$
    \Loop
    \State $\vec{g} \gets \nabla \mom{4}{C}$
    \State $\vec{w}_{new} = \vec{w} - \delta \vec{g}$
    \State normalize $\vec{w}_{new}$ as $\frac{\vec{w}_{new}}{\lVert \vec{w}_{new} \rVert}$
    \If{$mom_{4, \mat{C}}(\vec{w}_{new}) \geq mom_{4, \mat{C}(\vec{w})}$}
    \State \Return{$\vec{w}$}
    \Else 
    \State $\vec{w} \gets \vec{w}_{new}$
    \State go to step 3
    \EndIf
    \EndLoop
\end{algorithmic}
...
\begin{algorithmic}[1]
    ...
    \State $\vec{w}_{new} = \vec{w} + \delta \vec{g}$
    \State normalize $\vec{w}_{new}$ as $\frac{\vec{w}_{new}}{\lVert \vec{w}_{new} \rVert}$
    \If{$mom_{4, \mat{C}}(\vec{w}_{new}) \leq mom_{4, \mat{C}(\vec{w})}$}
    \State \Return{$\vec{w}$}
    \Else 
    \State $\vec{w} \gets \vec{w}_{new}$
    \State go to step 3
...
\begin{algorithmic}[1]
    ...
    \State $\vec{u}_{new} \gets \vec{u} - \delta \cdot \hat{\vec{m}}_t / (\sqrt{\hat{\vec{v}} + \epsilon})$
    \EndLoop
\end{algorithmic}
\end{algorithm}

Below is a more detailed description of the approach.
\begin{algorithm}[H]
\caption{Proposed version of attack}
\begin{algorithmic}[1]
...
guessing. The following table shows results of this.
...
\begin{table}[h]
    ...
    \begin{tabular}{lcccc}
        ...
        Attack on Hawk 128 & something & something \\
        \midrule
